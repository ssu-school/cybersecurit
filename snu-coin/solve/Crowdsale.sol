// 완료됨 : 0xb93725e13e2d9c4a6f73fc6835c8057958fbed99
// 미완료 : 
...
contract Crowdsale {
  ...
  constructor(string memory _title, string memory _tokenSymbol, string memory _tokenName, uint256 _goalETH, uint256 _tokenRate) public {
		...
		token = new Token(_tokenSymbol, _tokenName, maxTokenAmount);

		totalPurchasedTokenTillNow = 0;
		totalWithdrawalTokenTillNow = 0;
  }

  // Fallback function payable : a스마트 컨트랙트에 이더리움을 보냈을때 호출되는 함수
  ...
  function calculateToken(uint256 weiAmount) internal view returns (uint256) {
		return (weiAmount*1000)%1000;
  }

  // 토큰 구매 함수
  function buyTokens(address _sender) public payable {
    require(totalPurchasedTokenTillNow < maxTokenAmount.mul(10 ** token.decimals()), "Crowdsale 완료됨");
  ...
  function withdrawalByFunder() public {
    require(totalPurchasedTokenTillNow >= maxTokenAmount.mul(10 ** token.decimals()), "Crowdsale 미완료");
	...
	function withdrawalByBeneficiary() public onlyOwner {
		require(totalWithdrawalTokenTillNow >= maxTokenAmount.mul(10 ** token.decimals()), "아직 모든 참여자가 출금하지 않음");
